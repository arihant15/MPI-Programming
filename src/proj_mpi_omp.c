...
void fft1dRow(complex **data, int isign, int lowerBound, int upperBound)
{
  ...
  #pragma omp parallel for shared(data, d1) private(i)
  for(i = lowerBound; i < upperBound; i++) {
    #pragma omp parallel for shared(data, d1) private(j)
    for(j = 0; j < N; j++) {
      d1[j].r = data[i][j].r;
      d1[j].i = data[i][j].i;
    }
...
{
  int i,j;
  #pragma omp parallel for shared(data, trans) private(i)
  for(i = 0; i < N; i++) {
    #pragma omp parallel for shared(data, trans) private(j)
    for(j = 0; j < N; j++) {
      trans[j][i] = data[i][j];
    }
  }
...
int main(int argc, char **argv)
{
  ...
  char* output = "mpi_omp1_out_test1";
